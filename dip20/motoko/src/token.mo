...
(3620,1),(3941,1)]),
    ...
    private stable var claimTime: Int = 1440;
    ...
    */
    ...
    public shared(msg) func getNextClaimTime(tokenID : Nat) : async Int {
        let last_claim = canClaimEntries[tokenID];
        let now = Time.now();
        let difference = now - last_claim;
        var minutes = claimTime - ((difference / 1000000000) / 60);
        return minutes;
    };
            ...
            case (#ok(tokenlist)) {
                ...
                for (x in tokenlist.vals()) {
                    ...
                    if(minutes >= claimTime) {
                        ...
                        switch(how_much) {
                            case(?v) {
                                tokens_to_add += 50 * 1000;
                            };
                            case(_) {
                                tokens_to_add += 10 * 1000;
                            };
                        };
                    };
    ...
    */
    ...
    public shared(msg) func setclaimTime(_claimTime: Int) {
        assert(msg.caller == owner_);
